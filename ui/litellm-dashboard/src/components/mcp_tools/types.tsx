...
const ADMIN_CONFIG_CREDENTIAL_KEYS = ["upstream_resource"] as const;
...
// Admin-config keys show their real value on edit, so a blank one means "clear it", not the
// "keep existing" a blank secret means. Re-add each blank one as an explicit null so the merge drops it.
export const withAdminConfigClears = (
  payload: Record<string, unknown> | undefined,
  formCredentials: Record<string, unknown> | null | undefined,
): Record<string, unknown> | undefined => {
  const clears = Object.fromEntries(
    ADMIN_CONFIG_CREDENTIAL_KEYS.filter((key) => {
      const value = formCredentials?.[key];
      return value === undefined || value === null || value === "";
    }).map((key) => [key, null]),
  );
  const result = { ...(payload ?? {}), ...clears };
